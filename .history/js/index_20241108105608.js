document.addEventListener("DOMContentLoaded", function()  {
    ...
    const navbarItemActiveContent = document.querySelector(".navbar li.active > div").cloneNode(true);
    const navbarContent = document.querySelector(".profile-content");
    if (navbarItemActive) {
      navbarContent.inner(navbarItemActiveContent);
    }
    navbarItem.map((item, i, arr) => {
      item.addEventListener("click", function () {
        ...
        if(item.classList.contains('active')) {
          ...
          navbarContent.appendChild(item);
        }
      });
    });
